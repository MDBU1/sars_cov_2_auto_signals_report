...
from modules.setup_folders import ClassSetupFolders


# %%


class ClassSetupFileConditions(ClassSetupFolders):
    def __init__(self, class_signal):
        # super().__init__(class_signal.signal_no, class_signal.data_region, class_signal.lineage,
        # class_signal.mutations, class_signal.nucleotides, class_signal.df_logs, class_signal.logging_level,
        ...
        self.date_min, self.date_max = self.meth_select_date_parameter_method()
    ...
    def meth_return_custom_filename(self):
        if self.filename:
            temp_str_name = os.path.basename(self.filename)
    ...
    def meth_return_date_parameters_standard(self):
        ...
        # return date from 1 week ago since last Sunday !!!!!!!!!!!!!!!!!!!!! Just using from last Sunday !!!!!!!!!!
        val_date_latest = pd.to_datetime(val_date_last_sunday) - pd.to_timedelta(0, unit="d")
    ...
    def meth_return_date_parameters_specified(self):
        temp_date = self.date_begin
        # return date from user entry
        val_date_earliest = (pd.to_datetime(temp_date) - pd.Timedelta(days=((pd.to_datetime(temp_date).dayofweek - 6)
                                                                            % 7))).date()
        val_date_earliest = pd.to_datetime(val_date_earliest)
        val_date_latest = pd.to_datetime(val_date_earliest) + pd.to_timedelta(49, unit="d")
        self.logger.info(f"analysis date period: {val_date_earliest} - {val_date_latest}")
        return val_date_earliest, val_date_latest,

    def meth_select_date_parameter_method(self):
        if self.date_begin:
            val_date_earliest, val_date_latest = self.meth_return_date_parameters_specified()
            return val_date_earliest, val_date_latest
        else:
            val_date_earliest, val_date_latest = self.meth_return_date_parameters_standard()
            return val_date_earliest, val_date_latest
